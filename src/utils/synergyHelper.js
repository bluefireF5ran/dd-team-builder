...
export const analyzeSynergy = (heroes) => {
  ...
  if (coverage.size < 2) return { level, notes };
  ...
  if (duplicates.length) {
    notes.push(`Duplicate class: ${[...new Set(duplicates)].join(', ')}`);
    raise('warning');
  }

  const healers = coverage.countOf('heal');
  if (healers === 0) {
    notes.push('Nothing in the party heals HP');
    raise('warning');
  } else if (healers >= 3) {
    // Tres es exceso; dos es la norma de muchas comps buenas y avisar de eso
    // era ruido.
    notes.push(`Three or more heroes heal (${namesFor(coverage, 'heal')}) — the party may lack damage`);
    raise('warning');
  }

  if (coverage.size >= 4 && !coverage.has('stressHeal') && !coverage.has('camp:stressHeal')) {
    notes.push('No stress healing, in battle or in camp — long dungeons may be risky');
    raise('warning');
  }
  ...
  if (coverage.has('mark') && coverage.has('markPayoff')) {
    notes.push(`Mark synergy: ${namesFor(coverage, 'mark')} marks, ${namesFor(coverage, 'markPayoff')} cashes it in`);
  } else if (coverage.has('mark')) {
    notes.push(`${namesFor(coverage, 'mark')} marks, but nothing in the party hits harder on a marked target`);
    raise('warning');
  } else if (coverage.has('markPayoff')) {
    notes.push(`${namesFor(coverage, 'markPayoff')} hits harder on a marked target, but nothing here marks`);
    raise('warning');
  }
  ...
  if (coverage.size >= 4) {
    ...
    if (!pressure.length) {
      notes.push('No stun, blight or bleed — nothing but straight damage');
      raise('warning');
    }
  }
